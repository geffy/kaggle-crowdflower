...
logging.info("Mikhail's model_1 learner script")
...
logging.info("Collecting BoWs")
# collect BoW + TfIdf for query
...
# same for title (note -- train and test are intersected by words)
...
logging.info("Collect additional features")
feat_list = [
    u'w2v_sim',
    u'w2v_dist',
    u'tsne_title_1', 
...
logging.info("Load extended counts")
...
def make_mf_sliced_regression(subset_tr, subset_te, clf, n_round=3, target_col='median_relevance'):
    '''
    Perform per-query slicing, BoW on text, fit @clf and get prediction for test. Assumed that @clf -- regressor
    '''
    print '\n [make_mf_slice]'
    print clf
...
# bind features
logging.info('Assembling features for 2nd level')
X_train = np.hstack(
    (X_base_add, 
...
# fit main model
logging.info('Learning 2nd level model')
rfR = RandomForestRegressor(n_estimators=15000, n_jobs=-1, min_samples_split=3, random_state=42)
rfR.fit(X_train, y_base)
y_pred_rfR = rfR.predict(X_test)

# dump prediction
logging.info('Dumping prediction')
np.savetxt(cfg.path_processed + 'mikhail_model1.txt', y_pred_rfR)
